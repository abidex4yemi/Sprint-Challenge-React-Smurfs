...
- [X] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  *`client-side routing:` Client side routing is way of handling route in the browser or client side using the JavaScript code loaded in the browser. When user click a link or request for a resource in the browser the state of the app changes but request to the server is being prevented. since the page will not be refresh or reload this makes user experience great*

  *`server-side routing:` Server side routing is old or native way browser handle user request and every request is been requested from the server and this make the whole browser refreshes when a new resource is being requested*

- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

*`POST` create new record, `GET` get an existing record, `PUT OR PATCH` update a record, `DELETE` remove a record*
